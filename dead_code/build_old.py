...
def prospector():
    ...
    with safe_cd(SRC):
        command = (
            "{0} prospector {1} --profile {1}_style --pylint-config-file=.pylintrc --profile-path=.prospector".format(
                PIPENV, PROJECT_NAME
            )
            .strip()
            .replace("  ", " ")
...
def lint():
    ...
    with safe_cd(SRC):
        ...
        # command += "{0}--rcfile=.pylintrc {1}".format(django_bits, PROJECT_NAME).split(" ")
        command = (
            "{0} pylint {1} --rcfile=.pylintrc {2}".format(
...
def compile_mark_down():
    ...
    """
    print(
        "Not compiling README.md because moderately complex MD makes pypi rst parser puke."
    )
    # with safe_cd(SRC):
    #     if IS_TRAVIS:
...
def gemfury():
    ...
    cp = subprocess.run(
        ("fury login --as={0}".format(GEM_FURY).split(" ")),
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        shell=False,
        check=True,
        ...
        for kind in ["gz", "whl"]:
            try:
                ...
                for file_name in files:
                    cp = subprocess.run(
                        (
                            "fury push {0} --as={1}".format(file_name, GEM_FURY).split(
                                " "
                            )
                        ),
                        stdout=subprocess.PIPE,
...
def check_manifest():
    ...
    if num_lines > cutoff:
        print(
            "Too many lines of manifest problems : {0}, max {1}".format(
                num_lines, cutoff
            )
        )
        exit(-1)
