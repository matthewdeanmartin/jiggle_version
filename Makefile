...
test: clean uv.lock install_plugins
	...
	$(VENV) bash scripts/integration_test.sh
...
venv315:
	...
	uv pip install -e . pytest pytest-cov pytest-timeout pytest-mock hypothesis --python $(PY315_EXE)
...
test315: venv315
	...
	$(PY315_EXE) -m pytest test -q --timeout=120
...
dont-be-lazy:
	...
	@uv run dont_be_lazy --root . --no-color scan jiggle_version --no-config-suppressions || true

.PHONY: pydoc-docs
pydoc-docs:
	@uv run pydoc_fork jiggle_version -o ./pydoc/
