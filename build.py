...
try:
    ...
    if hasattr(sys, "real_prefix"):
        print("Pynt is not installed, run pipenv install --dev --skip-lock")
    else:
...
PIPENV = SECTION["PIPENV"]
# pylint: disable=simplifiable-if-expression
WANT_TO_USE_PIPENV = True if PIPENV else False
...
def check_command_exists(
    ...
    try:
            ...
            if command in ["pyroma", "liccheck", "pipenv_to_requirements", "pyupgrade"]:
                ...
                if is_powershell():
                    _ = subprocess.check_output(
                        ["pipenv", "run", "powershell", "get-command", command]
                    )
                else:
                    _ = subprocess.check_output(["pipenv", "run", "which", command])
            else:
                _ = subprocess.check_output(["pipenv", "run", command, "--version"])
        ...
        return False
    return True
    # doesn't work because pynt doesn't run in a pipenv environment.
    # if which(command) is not None:
    #     raise TypeError("{0} isn't installed, maybe run pipenv install --dev, or "
    #                     "activate virtual environment".format(command))
...
def prep_print_simple(command: str, no_project: bool = False) -> str:
    ...
    """
    if no_project:
        command = f"{PIPENV} {command}".strip().replace("  ", " ")
    else:
        command = f"{PIPENV} {command} {PROJECT_NAME}".strip().replace("  ", " ")
    print(command)
    return command
...
def validate_project_name() -> None:
    ...
    problems = 0
    if not found:
        print(f"Found more than no modules at all, did you forget __init__.py?")
        problems += 1
...
def pipenv_installs() -> None:
    ...
    pipenv is so slow
    """
    if PIPENV or PIPENV_ACTIVE or WANT_TO_USE_PIPENV:
        command_text = "pipenv run pipenv install --dev --skip-lock"
        print(command_text)
        command = shlex.split(command_text)
        execute(*command)
    else:
...
def gitchangelog() -> None:
    ...
    with safe_cd(SRC):
        command_text = f"{PIPENV} {command_name}".strip().replace("  ", " ")
        print(command_text)
        command = shlex.split(command_text)
...
def spell_check() -> None:
    ...
    def read_file() -> None:
        with open("spelling.txt", "r") as reading_file:
...
def register_scripts() -> None:
    ...
    # command = "pipenv install -e . --skip-lock"
    command_text = "pipenv run pip install -e ."
    print(command_text)
...
def total_loc() -> int:
    ...
    # pylint: disable=bare-except
    try:
        total_loc_value = open(".build_state/pygount_total_loc.txt").read()
    except:  # noqa: B001
        do_count_lines_of_code()
        total_loc_value = open(".build_state/pygount_total_loc.txt").read()
...
def do_count_lines_of_code() -> None:
    ...
    lines = sum(
        int(line.split("\t")[0]) for line in open(output_file_name) if line != "\n"
    )
...
@task(pipenv_installs, pyupgrade, compile_py)
...
def do_formatting(check: str) -> None:
    ...
    with safe_cd(SRC):
        command_text = f"{PIPENV} black {PROJECT_NAME} {check}".strip().replace(
            "  ", " "
        )
        print(command_text)
        command = shlex.split(command_text)
...
def do_pyroma() -> str:
    ...
    check_command_exists(command)
    with safe_cd(SRC):
        command = f"{PIPENV} pyroma --directory --min=8 .".strip().replace("  ", " ")
        print(command)
        execute(*(command.split(" ")))
...
def docker_lint() -> None:
    ...
    with open("Dockerfile") as my_input, open(
        ...
        command = f"docker run --rm -i hadolint/hadolint hadolint -".strip()
        _ = subprocess.run(command, stdin=my_input, stdout=my_output)
...
def yaml_lint() -> str:
    ...
    check_command_exists(command)
    with safe_cd(SRC):
        command = f"{PIPENV} yamllint {PROJECT_NAME}".strip().replace("  ", " ")
        print(command)
        execute(*(command.split(" ")))
...
def do_dodgy() -> str:
    ...
    warning_messages = json.loads(output)
    if warning_messages["warnings"]:
        print("Dodgy found problems")
        for message in warning_messages["warnings"]:
            print(message)
        exit(-1)
...
def precommit() -> None:
    ...
    with safe_cd(SRC):
        command_text = f"{PIPENV} pre-commit install".strip().replace("  ", " ")
        print(command_text)
        command = shlex.split(command_text)
        execute(*command)

        command_text = f"{PIPENV} pre-commit run --all-files".strip().replace("  ", " ")
        print(command_text)
        command = shlex.split(command_text)
        result = execute_get_text(command, ignore_error=True, env=config_pythonpath())
        assert result
...
def coverage() -> None:
    ...
    with safe_cd(SRC):
        ...
        # run again for min cov check (is this redundant now?)
        test_folder = "test"
        command = (
            f"{PIPENV} pipenv run py.test {test_folder} -v "
            f"--cov={PROJECT_NAME} "
            f"--cov-fail-under {MINIMUM_TEST_COVERAGE}".strip().replace("  ", " ")
        )
        print(command)
...
def coverage_report() -> None:
    ...
    with safe_cd(SRC):
        ...
        test_directory = "test"
        command = (
            f"{PIPENV} pipenv run py.test {test_directory} -v "
...
def do_docs() -> str:
    ...
    with safe_cd(SRC):
        with safe_cd("docs"):
            my_env = config_pythonpath()
            command = f"{PIPENV} make html".strip().replace("  ", " ")
            print(command)
            execute_with_environment(command, env=my_env)
...
def do_safety() -> str:
    ...
    with open(requirements_file_name, "w+") as out:
        subprocess.run(["pip", "freeze"], stdout=out, stderr=out)
...
def do_mypy() -> str:
    ...
    actually_bad_lines = []  # type: List[str]
    found_some_lines = False
    with open(mypy_file, "w+") as lint_file:
        lines = out.decode().split("\n")
        for line in lines:
            found_some_lines = True
    ...
    if num_lines == 0 and not found_some_lines:
        # should always have at least 'found 0 errors' in output
...
def do_pin_dependencies() -> None:
    ...
    with safe_cd(SRC):
        execute(*(f"{PIPENV} pipenv_to_requirements".strip().split(" ")))
    if not os.path.exists("requirements.txt"):
        print(
...
def do_dead_code() -> str:
    ...
    with safe_cd(SRC):
        command_text = f"{PIPENV} vulture {PROJECT_NAME}".strip().replace("  ", " ")
        print(command_text)
        command = shlex.split(command_text)
...
def call_check_manifest_command(output_file_name: str, env: Dict[str, str]) -> None:
    ...
    command_text = f"{PIPENV} check-manifest".strip().replace("  ", " ")
...
def do_check_manifest() -> str:
    ...
    with open(output_file_name) as outfile_reader:
        ...
        if not os.path.isfile("MANIFEST.in") and "no MANIFEST.in found" in text:
            command_text = f"{PIPENV} check-manifest -c".strip().replace("  ", " ")
            command = shlex.split(command_text)
...
def jiggle_version() -> None:
    print("Skipping because this conflicts with black")
    return
    # repo is a Repo instance pointing to the git-python repository.
    # For all you know, the first argument to Repo is a path to the repository
    # you want to work with
    if IS_GITLAB or IS_JENKINS:
        print("Won't jiggle version, can't detect what branch we're on")
        return
    repo = Repo(".")
    if str(repo.active_branch).strip() == "master":
        ...
        print(f"Current branch is {repo.active_branch}, won't auto-increase version")
...
def do_package() -> None:
    ...
    with safe_cd(SRC):
        ...
        try:
            ...
            with safe_cd(SRC):
                command_text = f"{PIPENV} twine check dist/*".strip().replace("  ", " ")
                print(command_text)
                command = shlex.split(command_text)
                execute(*command)
        except Exception as ex:
...
def check_package() -> None:
    ...
    check_command_exists("twine")
    with safe_cd(SRC):
        execute(*(f"{PIPENV} twine check dist/*".strip().split(" ")))
